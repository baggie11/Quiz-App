...
const QuizPlayPage = () => {
  ...
  const convertApiQuestionToQuestion = (apiQuestion: ApiQuestion): Question => {
    const rawType = String(apiQuestion.question_type || "").toLowerCase();
    ...
    let type: QType = typeMap[rawType] || 'quiz';
    
    // Extract just the option texts from question_options
    const optionsSource = Array.isArray(apiQuestion.options)
      ? apiQuestion.options
      : apiQuestion.question_options || [];
    const options = optionsSource.map((opt: any) =>
    ...
    if (Array.isArray(optionsSource)) {
      const correctIndex = optionsSource.findIndex((opt: any) =>
    ...
    return {
      ...
      type: type,
      options: options,
      ratingMin: apiQuestion.options?.min || 1,
      ratingMax: apiQuestion.options?.max || apiQuestion.rating_max,
      ratingLabels: apiQuestion.options?.labels,
